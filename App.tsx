...
import { LayoutDashboard, Users, BarChart3, Settings, Calendar as CalendarIcon, Bell, Map, Clock, Car, Zap, Database, Cloud, ExternalLink, Info } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // 初始化数据
  const [drivers, setDrivers] = useState<Driver[]>(() => storage.load<Driver[]>('DRIVERS') || generateDrivers());
  const [vehicles, setVehicles] = useState<Vehicle[]>(() => storage.load<Vehicle[]>('VEHICLES') || generateVehicles([]));
  const [tasks, setTasks] = useState<Task[]>(() => storage.load<Task[]>('TASKS') || []);
  const [driverSchedules, setDriverSchedules] = useState<DriverSchedule[]>(() => storage.load<DriverSchedule[]>('DRIVER_SCHEDULES') || generateSchedule(drivers, currentDate));
  const [vehicleSchedules, setVehicleSchedules] = useState<VehicleSchedule[]>(() => storage.load<VehicleSchedule[]>('VEHICLE_SCHEDULES') || generateVehicleSchedule(vehicles, currentDate));
        ...
        <nav className="flex-1 p-5 space-y-1.5 overflow-y-auto scrollbar-hide">
          ...
          <div className="pt-4 mt-4 border-t border-slate-800">
            ...
            >
              ...
              <span className="text-sm uppercase tracking-widest">部署与云同步</span>
            </button>
          </div>
        </nav>
        ...
        <header className="bg-white border-b border-slate-100 p-6 flex justify-between items-center z-10">
           <div className="flex items-center gap-6">
             <h1 className="text-2xl font-black text-slate-800 uppercase tracking-tight">
               ...
               {/* 其他标题省略以节省篇幅 */}
               {activeTab !== 'dashboard' && activeTab !== 'deploy' && '车队管理'}
             </h1>
             {activeTab === 'deploy' && (
               <div className="flex items-center gap-2 bg-orange-50 text-orange-600 px-4 py-1.5 rounded-full border border-orange-100 animate-pulse">
                  ...
                  <span className="text-xs font-black uppercase tracking-tighter">待执行云端绑定</span>
               </div>
             )}
           </div>
           
           <div className="flex items-center gap-4">
              <button onClick={() => setShowDeployGuide(!showDeployGuide)} className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-xl transition-all">
                <Cloud className="w-4 h-4 text-slate-600" />
                <span className="text-xs font-black text-slate-600">部署文档</span>
              </button>
                ...
                <div className="space-y-6">
                  {[
                    ...
                    <div key={s.step} className="flex gap-6 p-6 rounded-3xl border border-slate-50 bg-slate-50/30 hover:bg-white hover:shadow-xl transition-all group">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black ${s.done ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-400'}`}>
                        {s.step}
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-slate-900 rounded-[40px] p-8 text-white">
                  ...
                  <p className="text-slate-400 text-sm leading-relaxed">
                    Cloudflare D1 数据库在免费额度内支持 <span className="text-white font-bold">500MB</span> 数据存储。
                    对于您的司机流水，这相当于可以存储约 <span className="text-white font-bold">100 万条</span> 订单记录，初期完全免费。
                  </p>
                </div>
                <div className="bg-indigo-900 rounded-[40px] p-8 text-white">
                  ...
                  <p className="text-slate-400 text-sm leading-relaxed">
                    本应用使用的 Gemini 3 Flash 模型提供慷慨的免费额度。
                    在正常运营频率下（每天分析几次报表），您的 API 费用预计为 <span className="text-white font-bold">¥0</span>。
                  </p>
                </div>
              </div>
            </div>
