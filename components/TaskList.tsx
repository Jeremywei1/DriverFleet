...
import { MapPin, Calendar, Truck, ArrowRight, Trash2, AlertCircle } from 'lucide-react';
...
const TaskList: React.FC<Props> = ({ tasks, drivers, onDeleteTask }) => {
  ...
  const handleDelete = (task: Task) => {
    // 优先使用任务对象自带的 date，否则从 startTime 提取
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-hide">
        {tasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-slate-300">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.2em]">今日暂无调度数据</p>
          </div>
        ) : (
          tasks.map(task => {
            ...
            return (
              <div key={task.id} className="group relative border border-slate-100 rounded-[28px] p-6 hover:shadow-xl hover:border-indigo-100 transition-all bg-white overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-4 pr-10">
                  <div>
                    <h3 className="font-black text-slate-800 italic uppercase tracking-tight">{task.title}</h3>
                ...
                <div className="flex items-center gap-2 mb-4 bg-slate-50 p-4 rounded-2xl text-[11px] font-bold text-slate-500 border border-slate-100/50">
                   <div className="flex-1 truncate flex items-center">
                      <MapPin className="w-3 h-3 mr-2 text-indigo-400" /> {task.locationStart}
                   </div>
                   <ArrowRight className="w-3 h-3 text-slate-300 mx-1" />
                   <div className="flex-1 truncate flex items-center justify-end">
                      {task.locationEnd} <MapPin className="w-3 h-3 ml-2 text-rose-400" />
                   </div>
                </div>

                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                     <div className="w-8 h-8 rounded-xl bg-indigo-50 flex items-center justify-center text-[10px] font-black text-indigo-500 border border-indigo-100 uppercase">
